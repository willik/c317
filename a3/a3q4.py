...
class Variable:
    ...
    def copy(self):
        ...
        var_cp = Variable(self.domain.copy())
...
class LatinSqProblem:
    ...
    def create_initial_state(self):
        ...
        # make a list where each element is a list of entries in a column
        columns = []
        for i in range(0, self.order):
            col = [row[i] for row in self.square]  # construct column by grabbing i-th entry of each row
            columns.append(col)
        ...
        for row_num, row in enumerate(self.square):
            for col_num, entry in enumerate(row):
                ...
                if entry != 0:
                    # dont bother figuring out the domain if the entry is not a blank
                    new_var = Variable(list(range(1, order + 1)))
                    new_var.assign(entry)
                    variables.update({(row_num, col_num): new_var})
                else:
                    # entry is a blank, so figure out the domain
                    full_domain = []

                    # start with all possible domain values
                    full_domain.extend(list(range(1, order + 1)))

                    # remove assigned values in current row
                    partial_domain_row = [x for x in full_domain if x not in row]

                    # remove assigned values in current column
                    new_domain = [x for x in partial_domain_row if x not in columns[col_num]]

                    # create the variable
                    new_var = Variable(new_domain)

                    blanks.append((row_num, col_num))  # build list of identifiers of unassigned variables
    ...
    def result(self, state, action):
        ...
        # forward checking on row
        keys_to_check = []

        # add keys corresponding to same row as the variable
        for col_num in range(0, self.order):
            key = (action[0][0], col_num)
            keys_to_check.append(key)

        # add keys corresponding to same column as the variable
        for row_num in range(0, self.order):
            key = (row_num, action[0][1])
            keys_to_check.append(key)

        # assume the new state will be consistent
        new_state_consistent = True

        # do the forward check
        for key in keys_to_check:
            curr_var = new_vars.get(key)
            if curr_var.current == None:
                # only forward check unassigned variables
                try:
                    # remove the value from domain
                    curr_var.domain.remove(action[1])
                except:
                    # do nothing if the value isn't there
                    pass

                if len(curr_var.domain) == 0:
                    # the domain is empty now, mark state inconsistent
                    new_state_consistent = False

        # make the new state
        new_state = LatinSqState(new_vars, new_blanks)

        if not new_state_consistent:
            new_state.consistent = False
